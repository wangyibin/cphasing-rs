...
fn map_seq_to_seqs_by_command(
    read_id: &String,
    mod_seq: &[u8],
    targets: Vec<SeqRecord>,
    ...
    let ref_seqs = targets
        .iter()
        .map(|x| {
            format!(
                ">{}\n{}\n",
                x.name,
                String::from_utf8(x.seq.to_vec()).unwrap()
            )
        })
        .collect::<Vec<String>>()
        .join("");
...
fn map_seq_to_seqs_by_edit_distance(
    read_id: &String,
    mod_seq: &[u8],
    targets: Vec<SeqRecord>,
    ...
    // convert &u8 to u8
    let mod_seq = mod_seq.iter().map(|x| **x).collect::<Vec<u8>>();
    ...
    for i in 0..targets.len() {
        ...
        let target_seq = target
            ...
            .collect::<Vec<&u8>>();
        let target_seq = target_seq.iter().map(|x| **x).collect::<Vec<u8>>();
...
fn map_seq_to_seqs_by_local(
    ...
    let match_fn = |a: u8, b: u8| {
        if a == b'm' {
            if b == b'm' { 4i32 } else { -5i32 }
        } else {
            if b == b'm' {
                -5i32
            } else {
                if a == b { 2i32 } else { -5i32 }
            }
        }
    };
    ...
    for i in 0..targets.len() {
        ...
        //     }
        // }).collect::<Vec<&u8>>();
        let mut target_seq = target.seq.iter().map(|x| *x).collect::<Vec<u8>>();
        let target_seq = String::from_utf8(target_seq.to_vec()).unwrap();

        let mut aligner =
            Aligner::with_capacity_and_scoring(mod_seq.len(), target_seq.len(), scoring);
...
fn map_seq_to_seqs_by_banded_semiglobal(
    read_id: &String,
    mod_seq: &[u8],
    targets: Vec<SeqRecord>,
) -> anyResult<Vec<(usize, i32)>> {
    ...
    for i in 0..targets.len() {
        ...
        let mut aligner =
            Aligner::with_capacity_and_scoring(target_seq.len(), mod_seq.len(), scoring, 15, 5);
...
fn map_seq_to_seqs_by_block_aligner(
    read_id: &String,
    mod_seq: &[u8],
    targets: Vec<SeqRecord>,
) -> anyResult<Vec<(usize, i32)>> {
    ...
    // convert &u8 to u8
    let mut mod_seq = mod_seq.iter().map(|x| **x).collect::<Vec<u8>>();
    ...
    for i in 0..targets.len() {
        ...
        let mut target_seq = target_seq.iter().map(|x| **x).collect::<Vec<u8>>();
    ...
    if !au.is_empty() {
        ...
        'outer: for (p, s) in au.Primary.iter().zip(au.Secondary.iter()) {
            ...
            if p.mapq() >= min_quality || s.len() == 0 {
                ...
                let query_seq =
                    &mut seq[p_parser.query_start as usize..p_parser.query_end as usize];
                ...
                let align_results =
                    map_seq_to_seqs_by_banded_semiglobal(&read_id, &mod_seq, targets.clone())
                        .unwrap();
...
impl RecordParser {
    pub fn parse(r: &Record, h: &HeaderView, seq_len: u64) -> RecordParser {
        let cigar = r.cigar();
        let target = str::from_utf8(h.tid2name(r.tid() as u32)).unwrap();
        let target_start = r.pos();
        let target_end = cigar.end_pos();
        let mut query_start = cigar.leading_hardclips() + cigar.leading_softclips();
        let mut query_end = cigar.leading_hardclips()
            + 1
            + cigar
                .read_pos(target_end as u32 - 1, false, false)
                .unwrap()
                .unwrap() as i64;
    ...
    for r in bam.records() {
        ...
        if old_read_id != read_id {
            if old_read_id != "" {
                ...
                do_some(&au, seqs, &bam_header, min_quality, min_prob, &mut writer);
            }
