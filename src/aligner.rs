...
impl AlignmentUnit {
    ...
    fn do_some(&mut self, seqs: &HashMap<String, String>, bam_header: &HeaderView, min_quality: u8, writer: &mut Writer) {
       
        if !self.is_empty() {
            
            'outer: for (p, s) in self.Primary.iter().zip(self.Secondary.iter()) {
                ...
                if p.mapq() >= min_quality || s.len() == 0 {
                    ...
                    // let mut targets: HashMap<String, Vec<u8>> = HashMap::new();
                    // match p.aux(b"AS") {
                    //     Ok(value) => {
                    ...
                    // let score = calculate_alignment_score_from_cigar_string_view(p.cigar());
                    // println!("{} {} {}", self.read_id(), p.mapq(), score);
                    ...
                    match sub_seq(&seqs, &p_parser.target, 
                        ...
                        Some(seq) => {
                            if p_parser.is_reverse {
                                ...
                                targets.push(seq);
                                // targets.insert(format!("{}_{}", p_parser.target, &p_parser.target_start.to_string()), seq);
                            } else {
                                // targets.insert(format!("{}_{}", p_parser.target, &p_parser.target_start.to_string()), seq.as_bytes().to_vec());
                                targets.push(seq.as_bytes().to_vec());
                            }
                        
                        },
                        None => log::debug!("{}: {:?}", self.read_id(), p_parser),
                    };
                    ...
                    for ss in s {
                        ...
                        match sub_seq(&seqs, &ss_parser.target, 
                            ...
                            None => log::debug!("{}: {:?}", self.read_id(), ss_parser),
                            }; 
                    }
                    
                    // align modified seq to modified reference by meth-minimap2 (rust)
                    let mut align_results = Vec::new();
                    // if self.read_id() == "00dcbff1-f4ea-48e2-ac36-809f4f4f6663" {
                    //     println!(">{}\n{}", self.read_id(), String::from_utf8(mod_seq.to_vec()).unwrap());
                    
                    for (target, seq) in targets.iter().enumerate() {
                        // println!(">{}\n{}", target.to_string(), String::from_utf8(seq.to_vec()).unwrap());
                        let aligner = Aligner::builder()
                        .with_seq_and_id(&seq, target.to_string().as_bytes())
                        .unwrap()
                        .with_cigar();
    ...
    for r in bam.records() {
        ...
        if old_read_id != read_id {
            if old_read_id != "" {
                ...
                au.do_some(seqs, &bam_header, min_quality, &mut writer);
