...
use crate::sketch::{MinimizerData, MinimizerInfo, sketch};
...
impl AllelesFasta {
    pub fn seqs(&mut self) -> anyResult<Vec<Vec<u8>>> {
        ...
        read_process_fastx_records(
            ...
            |record, seq| {
                // runs in worker
                *seq = record.seq_lines().fold(Vec::new(), |mut s, seq| {
    ...
    fn collect_minimizers(&mut self, k: usize, w: usize) -> anyResult<Vec<MinimizerData>> {
        ...
        let mut minimizers: Vec<MinimizerData> = seqs
            .par_iter()
            .enumerate()
            .flat_map(|(i, seq)| {
    ...
    fn collect_anchors<'a>(
        ...
        k: usize,
        w: usize,
        mut minimizers: Vec<MinimizerData>,
        ...
        let groups = minimizers
            .iter_mut()
            .chunk_by(move |x| x.minimizer)
            .into_iter()
    ...
    fn calculate_simularity<'a>(
        ...
        let mut anchors = anchor
            .iter()
            .chunk_by(|x| (x.rid1, x.rid2, x.rev))
            .into_iter()
    ...
    pub fn run(&mut self, k: usize, w: usize, m: f64, output: &String) {
        ...
        let (anchor, unique_minimizer_count_db, contig_minimizer_count_db) =
            self.collect_anchors(k, w, minimizers).unwrap();
