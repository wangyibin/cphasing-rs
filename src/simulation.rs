...
use noodles::{core::Position, core::Region, vcf};
...
use rust_htslib::bam::{
    ...
    record::AuxArray, record::Cigar, record::CigarStringView,
};
use std::path::Path;

use crate::core::{BaseTable, ChromSizeRecord, common_writer};
...
impl RecordParser {
    pub fn parse(r: &Record, h: &HeaderView, seq_len: u64) -> RecordParser {
        let cigar = r.cigar();
        let target = std::str::from_utf8(h.tid2name(r.tid() as u32)).unwrap();
        let target_start = r.pos();
        let target_end = cigar.end_pos();
        let mut query_start = 0;
        let mut query_end = cigar
...
pub fn simulation_from_split_read(input_bam: &String, output_bam: &String, min_quality: u8) {
    ...
    'outer: for r in bam.records() {
        ...
        for _ in 0..count * 2 {
            ...
            if random_pos < &r_parser.query_start as &i64 + 500
                || random_pos > &r_parser.query_end as &i64 - 500
            {
                continue;
            }
        ...
        let mut mm_list = match record.aux(b"MM") {
            Ok(Aux::String(v)) => v.strip_suffix(";").unwrap().split(",").collect::<Vec<_>>(),
            _ => match record.aux(b"Mm") {
                Ok(Aux::String(v)) => v.strip_suffix(";").unwrap().split(",").collect::<Vec<_>>(),
                _ => todo!(),
            },
        };
        ...
        let ml_list = match record.aux(b"ML") {
            Ok(Aux::ArrayU8(v)) => v.iter().map(|x| x as u16).collect::<Vec<_>>(),
            _ => match record.aux(b"Ml") {
                Ok(Aux::ArrayU8(v)) => v.iter().map(|x| x as u16).collect::<Vec<_>>(),
                _ => todo!(),
            },
        };
        ...
        for (i, (a, b)) in pos_list_1.iter().zip(pos_list_2.iter()).enumerate() {
            ...
            if new_mm_list.len() != 0 {
                ...
                new_record
                    .push_aux(b"ML", Aux::ArrayU8(ml_aux_array))
                    .unwrap();
            }
...
pub fn simulate_porec(fasta: &String, vcf: &String, bed: &String, output: &String) {
    ...
    let mut bed_reader = csv::ReaderBuilder::new()
        ...
        .from_path(bed)
        .unwrap();
    let mut vcf_reader = vcf::indexed_reader::Builder::default()
        .build_from_path(vcf)
        .unwrap();
    ...
    for record in bed_reader.records() {
        ...
        if prev_read_idx != read_idx.to_string() && prev_read_idx != "" {
            writer
                .write_all(
                    format!(">{}_{}\n{}\n", prev_read_idx, vcf_prefix, read.join("")).as_bytes(),
                )
                .unwrap();
...
pub fn simulate_hic(
    ...
    let mut idx = 0;
    let mut vcf_reader1 = vcf::indexed_reader::Builder::default()
        .build_from_path(vcf)
        .unwrap();
    let mut vcf_reader2 = vcf::indexed_reader::Builder::default()
        .build_from_path(vcf)
        .unwrap();
    ...
    while let Some(r) = bam.records().next() {
        ...
        let chrom1 = std::str::from_utf8(header.tid2name(record.tid().try_into().unwrap()))
            .unwrap()
            .to_string();
        let chrom2 = std::str::from_utf8(header.tid2name(record2.tid().try_into().unwrap()))
            .unwrap()
            .to_string();
        ...
        let pos_hash1: HashMap<usize, usize> = pos_vec1
            .iter()
            .enumerate()
        ...
        let pos_hash2: HashMap<usize, usize> = pos_vec2
            .iter()
            .enumerate()
            .map(|(i, &x)| (x as usize, i))
            .collect();
        ...
        writer1
            .write_all(format!(">{}_{}\n{}\n", idx, vcf_prefix, read1.join("")).as_bytes())
            .unwrap();
        writer2
            .write_all(format!(">{}_{}\n{}\n", idx, vcf_prefix, read2.join("")).as_bytes())
            .unwrap();
    }
}
