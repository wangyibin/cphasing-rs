...
use crate::alleles::{AlleleRecord, AlleleStrandRecord, AlleleStrandTable, AlleleTable};
...
impl Pruner {
    ...
    pub fn prune(&mut self, whitehash: &HashSet<&String>, writer: &mut Box<dyn Write + Send>) {
        ...
        let contact_data = self.contacts.to_data(
            &unique_min,
        ...
        let all_prune_line_records = self
            .alleletable
            .allele_records
            .par_iter()
            .map(|record| {
                ...
                for contig in record.data.iter() {
                    'inner: for mut query_contig in allcontigs.iter() {
                        ...
                        if remove_db
                            .get(contig_pair.Contig1)
                            .map_or(false, |v| v.contains(contig_pair.Contig2))
                        {
                            continue 'inner;
                        }
        ...
        cross_allelic_contig_pairs
            .retain(|contig_pair| !allelic_contig_pairs.contains(contig_pair));
            ...
            match allele_strand_info.get(&contig_pair) {
                Some(allele_strand_record) => {
                    writeln!(
                        ...
                        contig_pair.Contig1,
                        contig_pair.Contig2,
                        allele_strand_record.length1,
                        allele_strand_record.length2,
                        allele_strand_record.matches,
                        allele_strand_record.identity,
                        0
                    )
                    .unwrap();
                }
                None => {
                    writeln!(
                        writer,
                        "{}\t{}\t{}\t{}\t{}\t{}\t{}",
                        contig_pair.Contig1, contig_pair.Contig2, 0, 0, 0, 0.98, 0
                    )
                    .unwrap();
                }
            }
        ...
        for contig_pair in cross_allelic_contig_pairs.into_iter() {
            ...
            writeln!(
                writer,
                "{}\t{}\t{}\t{}\t{}\t{}\t{}",
                contig_pair.Contig1, contig_pair.Contig2, 0, 0, 0, 0, 1
            )
            .unwrap();
        }
    }
    ...
    pub fn kprune(
        ...
        if whitehash.len() != 0 {
            contig_pairs.retain(|contig_pair| {
                whitehash.contains(contig_pair.Contig1) && whitehash.contains(contig_pair.Contig2)
            });
        }
            ...
            .filter_map(|contig_pair| {
                ...
                'outer: for group1 in alleles1.iter() {
                    ...
                    'inner: for group2 in alleles2.iter() {
                        ...
                        let mut matrix =
                            Matrix::new(group1_length, group2_length, OrderedFloat(0.0));
            ...
            match allele_strand_info.get(&contig_pair) {
                ...
                None => {
                    writeln!(
                        writer,
                        "{}\t{}\t{}\t{}\t{}\t{}\t{}",
                        contig_pair.Contig1, contig_pair.Contig2, 0, 0, 0, 0.75, 0
                    )
                    .unwrap();
                }
            }
