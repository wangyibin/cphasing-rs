...
impl Concatemer {
    ...
    pub fn parse_singleton(&mut self) {
        if let Some(i) = self
            .filter_reasons
            .iter()
            .position(|&r| r == FilterReason::Pass)
        {
...
impl PAFTable {
    ...
    pub fn parse2(&self, secondary: bool) -> Result<impl Iterator<Item = PAFLine>, Box<dyn Error>> {
        let input = common_reader(&self.file);
        let iter = input.lines().filter_map(move |l| {
    ...
    pub fn paf2table_v0_3_0(
            ...
            handles.push(thread::spawn(move || {
                ...
                fn process_group_vec(
                    ...
                    for (i, grp) in groups.iter_mut().enumerate() {
                        ...
                        process_single_group(
                            grp, ih, is_fd, is_fe, max_len, max_ord, buf, lp, ls, ll, lc,
                        );
                    }
                    groups.clear();
                }
                ...
                while let Ok(block) = rx.recv() {
                    ...
                    while start < len {
                        ...
                        let qlen = match iter.next().and_then(fast_parse_int) {
                            Some(v) => v as u32,
                            None => continue,
                        };
                        let qs = match iter.next().and_then(fast_parse_int) {
                            Some(v) => v as u32,
                            None => continue,
                        };
                        let qe = match iter.next().and_then(fast_parse_int) {
                            Some(v) => v as u32,
                            None => continue,
                        };
                        let strand_char = match iter.next() {
                            Some(v) => {
                                if v.is_empty() {
                                    '+'
                                } else {
                        ...
                        let t_bytes = match iter.next() {
                            Some(v) => v,
                            None => continue,
                        };
                        let tlen = match iter.next().and_then(fast_parse_int) {
                            Some(v) => v as u64,
                            None => continue,
                        };
                        let ts = match iter.next().and_then(fast_parse_int) {
                            Some(v) => v as u64,
                            None => continue,
                        };
                        let te = match iter.next().and_then(fast_parse_int) {
                            Some(v) => v as u64,
                            None => continue,
                        };
                        let matches = match iter.next().and_then(fast_parse_int) {
                            Some(v) => v as u32,
                            None => continue,
                        };
                        let alen = match iter.next().and_then(fast_parse_int) {
                            Some(v) => v as u32,
                            None => continue,
                        };
                        let mapq = match iter.next().and_then(fast_parse_int) {
                            Some(v) => v as u8,
                            None => continue,
                        };
                        ...
                        let q_cov = qe - qs;
                        let ident = if alen > 0 {
                            matches as f32 / alen as f32
                        } else {
                            0.0
                        };
                        let fr = if q_cov < min_length || ident < min_identity {
                            LOW_MQ_STR
                        } else {
                            PASS_STR
                        };
...

        {
            ...
            loop {
                ...
                if bytes_read == 0 {
                    if valid_len > 0 {
                        tx_raw.send(buffer[..valid_len].to_vec()).unwrap();
                    }
                    break;
                }
                ...
                let prev_nl = safe_slice[..last_nl]
                    .iter()
                    .rposition(|&b| b == b'\n')
                    .map(|p| p + 1)
                    .unwrap_or(0);
                let last_line = &safe_slice[prev_nl..last_nl];
                let tab_pos = last_line
                    .iter()
                    .position(|&b| b == b'\t')
                ...
                let scan_limit = if safe_slice.len() > 1024 * 1024 {
                    safe_slice.len() - 1024 * 1024
                } else {
                    0
                };
                ...
                while scan_end > scan_limit {
                    ...
                    let start_idx = safe_slice[..newline_idx]
                        .iter()
                        .rposition(|&b| b == b'\n')
                        .map(|p| p + 1)
                        .unwrap_or(0);
        ...
        log::info!(
            "Successful output Pore-C table `{}` (block-io-atomic-ids)",
            output
        );
        Ok(())
    }
        ...
        for _ in 0..num_threads {
            ...
            handles.push(thread::spawn(move || {
                ...
                fn parse_line_only(
                    ...
                    let strand_c = iter
                        .next()
                        .map(|s| if s.is_empty() { '+' } else { s[0] as char })
                        .unwrap_or('+');
            ...
            handles.push(thread::spawn(move || {
                ...
                while let Ok(data) = receiver.recv() {
                    ...
                    while start_idx < data.len() {
                        ...
                        if iter.nth(4).is_none() {
                            continue;
                        }
                        let target_bytes = match iter.next() {
                            Some(v) => v,
                            None => continue,
                        };
                        ...
                        let t_start_bytes = match iter.next() {
                            Some(v) => v,
                            None => continue,
                        };
                        let t_end_bytes = match iter.next() {
                            Some(v) => v,
                            None => continue,
                        };
                        ...
                        let mapq_bytes = match iter.next() {
                            Some(v) => v,
                            None => continue,
                        };
                        let mapq = match fast_parse_u64(mapq_bytes) {
                            Some(v) => v as u8,
                            None => continue,
                        };
                        ...
                        let t_start = match fast_parse_u64(t_start_bytes) {
                            Some(v) => v,
                            None => continue,
                        };
                        let t_end = match fast_parse_u64(t_end_bytes) {
                            Some(v) => v,
                            None => continue,
                        };
...
Please sort/group by 1st column before downsampling, e.g.:\n\
  zcat in.paf.gz | sort -k1,1 -S 4G --parallel=8 | gzip -c > in.qsorted.paf.gz",
    ...
    pub fn downsample_by_vpc(
        ...
        let mut flush_block = |block_lines: &mut Vec<String>,
                               cur_order: usize,
                               cur_qlen: u64,
                               rng: &mut StdRng,
                               wtr: &mut dyn Write,
    ...
    pub fn downsample_by_vpc_multi(
        ...
        fn parse_line(
            ...
            let eligible = mapq >= min_quality && qcov >= min_length && identity >= min_identity;
        ...
        fn process_block(
            ...
            let eligible_read = target_bases || (order >= min_order && order < max_order);
        ...
        // -----------------------------
        log::info!(
            "PAF downsample pass2 producer-consumer writing `{}`...",
            output
        );
    ...
    pub fn phase_reads(
        ...
        let mut emit_read =
            |qname: &str, scores: &mut HashMap<String, u32>, w: &mut dyn Write| -> anyResult<()> {
