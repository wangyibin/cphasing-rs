...
lazy_static! {
    static ref PARSE_TAG: Regex = Regex::new("(..):(.):(.*)").unwrap();
}
...
impl fmt::Display for PAFLine {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n",
               self.query, self.query_length, self.query_start, self.query_end, self.query_strand,
               self.target, self.target_length, self.target_start, self.target_end,
               self.match_n, self.alignment_length, self.mapq, self.NM, self.ms, self.AS, self.nn, self.tp)
    }
}
