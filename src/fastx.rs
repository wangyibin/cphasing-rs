...
impl Fastx {
    pub fn get_chrom_size(&self) -> AnyResult<HashMap<String, u64>> {
        let mut reader =
            needletail::parse_fastx_file(&self.file).map_err(|e| anyhow::anyhow!(e.to_string()))?;
    ...
    pub fn count_re(
        &self,
        patterns: &String,
    ) -> AnyResult<(hashHashMap<String, u64>, hashHashMap<String, u64>)> {
    ...
    pub fn digest(
        ...
        patterns: &String,
        slope: i64,
    ) -> AnyResult<HashMap<String, Vec<Vec<i64>>>> {
        ...
        for pattern in pattern_vec {
            ...
            log::info!(
                "Identifing restriction site positions `{}` in `{}`",
                pattern_uppercase,
                self.file
            );
                ...
                while let Ok((id, seq, start_bound, end_bound)) = rx.recv() {
                    ...
                    while curr < end_bound {
                        ...
                        if batch_buffer.len() > 14 * 1024 * 1024 {
                            if tx_w.send(batch_buffer).is_err() {
                                return;
                            }
                            batch_buffer = rx_f.recv().unwrap();
                        }

                        if end == seq.len() || curr + step >= end_bound {
                            break;
                        }
                        curr += step;
                    }
    ...
    pub fn slide(
        ...
        let file_type = match filetype {
            ...
            _ => get_file_type(&self.file).unwrap(),
        };
                ...
                for result in reader.records() {
                    ...
                    while start < seq_length {
                        ...
                        let record = match coordinate_suffix {
                            false => Record::with_attrs(
                                format!("{}_{}", record.id(), i).as_str(),
                                None,
                                seq_window,
                                qual_window,
                            ),
                            true => Record::with_attrs(
                                format!("{}:{}-{}", record.id(), start, end).as_str(),
                                None,
                                seq_window,
                                qual_window,
                            ),
            ...
            FileType::Fasta => {
                ...
                for result in reader.records() {
                    ...
                    while start < seq_length {
                        ...
                        let record = match coordinate_suffix {
                            false => bio::io::fasta::Record::with_attrs(
                                format!("{}_{}", record.id(), i).as_str(),
                                None,
                                seq_window,
                            ),
                            true => bio::io::fasta::Record::with_attrs(
                                format!("{}:{}-{}", record.id(), start, end).as_str(),
                                None,
                                seq_window,
                            ),
    ...
    pub fn kmer_count(&self, k: usize) -> AnyResult<HashMap<String, u64>> {
        ...
        for result in reader.records() {
            ...
            for i in 0..(seq_length - k + 1) {
                ...
                *kmer_count
                    .entry(String::from_utf8(rev_kmer).unwrap())
                    .or_insert(0) += 1;
            }
        }
        ...
        for result in reader.records() {
            ...
            for i in 0..(seq_length - k + 1) {
                ...
                if kmer_set.contains(kmer) {
                    ...
                    let rev_kmer =
                        String::from_utf8(bio::alphabets::dna::revcomp(kmer.bytes())).unwrap();
    ...
    pub fn mask_high_frequency_kmer(
        &self,
        k: usize,
        threshold: u64,
        output: &String,
...
pub fn split_fastq(
    ...
    log::info!(
        "write {} records to {}",
        &record_num,
        format!("{}_{}.fastq.gz", output_prefix, j)
    );
